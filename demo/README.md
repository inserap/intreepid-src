...
| Brique #6 — robustesse d'échelle spatiale | [`brique-6-robustesse-echelle.md`](brique-6-robustesse-echelle.md) | test H3 multi-résolution (6/7/8), verdict **robuste** sur 267K points OFROU + STATPOP, part unpopulated (corridors transit), caveats planaire/réseau, agnostique au domaine | à jour (`v0.7.0`) |
...
| Brique #11 — questions groupées et fiche dense | [`brique-11-questions-groupees.md`](brique-11-questions-groupees.md) | toutes les questions posées en **un** tour et servies une par une sans appel LLM (5 appels → 2), schéma de fiche prescrit dans la charte (8 clés racine, vocabulaire de colonne fixe), couverture par regroupement, second artefact `<dataset>.questions.yaml` (dialogue de ratification), attribution en trois postes | **gate échoué** (`v0.14.0`) — 2 critères sur 7 ; le mécanisme passe (5 appels → 2), seule la densité de la fiche échoue |
